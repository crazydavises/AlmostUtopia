...
{		
	...
	function InitDB()
	{
		...
		$dayStats = $wpdb->get_row( "SELECT current_day, viewing_day FROM au_game_state");
		...
		$this->army_properties = $wpdb->get_results("SELECT description, name, plural, field_name, unit, cost_per_unit, resell_per_unit, value_per_unit, loss_unit, loss_print, trader_table, value_pretty_print FROM au_army_properties");
		$this->industry_properties = $wpdb->get_results("SELECT description, name, plural, field_name, unit, cost_per_unit, resell_per_unit, trader_table, value_per_unit, value_pretty_print FROM au_industry_properties");
		$this->bank_properties = $wpdb->get_results("SELECT Name, pay_field_name, received_field_name, readonly FROM au_bank_properties");
		$this->entities = $wpdb->get_results("SELECT Name, bank_table, external_bank_field, internal_paid, internal_received, aggregate, link FROM au_entities");
		$this->consumables = $wpdb->get_results("SELECT Name, field_name, production_field, units_pretty_print, formula FROM au_consumption_properties ORDER BY id");
		$this->diplomat_properties = $wpdb->get_results("SELECT name, field_name, units_pretty_print FROM au_diplomat_properties");
		$this->taxes = $wpdb->get_results("SELECT Name, field_name, money_recd_field_name FROM au_tax_types");
		$this->population = $wpdb->get_results("SELECT Name, field_name, readonly FROM au_population_properties");
		$this->scenarios = $wpdb->get_results("SELECT id, Title, scene, Notes FROM au_scenarios WHERE day=" . $this->viewDay );
		$this->weathers = $wpdb->get_results("SELECT id, description FROM au_available_weather");
		$this->arms_trader_properties = $wpdb->get_results("SELECT description, name, plural, field_name, unit, value_per_unit, value_pretty_print FROM au_arms_trader_properties");
		$this->real_estate_properties = $wpdb->get_results("SELECT description, name, plural, field_name, unit, value_per_unit, value_pretty_print FROM au_real_estate_properties");
	...
	function updateValue( $table, $entity, $day, $field, $new_quant, $relative)
	{
		$val = $new_quant;		
		$cur_data = $wpdb->get_row("SELECT day, " . $field . " from " . $table . " WHERE day=". $day );
		$output = '';
		...
		if ($cur_data)
		{
			...
			{
					$wpdb->update($table, array( $field => $val),
												array('day' => $day),
												array('%d'),
												array('%d'));
			}
...
//			else
	...
	function LoadDiplomat( $atts)
	{
		...
		$output .= '<br><strong>' . $country . ' Diplomat Day ' . $this->viewDay . ' </strong><br /><br />';
		...
		if( isset($_POST['add']) )
		{
			$balID = $wpdb->get_var("SELECT MAX(id), balance FROM $tableName");
			$oldBal = $wpdb->get_var("SELECT balance FROM $tableName WHERE id=$balID");
	...
	//	$selection_fields = $this->getSelectionFields($country, 'diplomat', true);		
 		$theData = $wpdb->get_results("SELECT action, quantity, item_name, other_party, price, balance FROM " . $tableName . " WHERE day = " . $this->viewDay . " ORDER by id" );
		if($theData)
		{
			$output .= '<br><strong>Transactions</strong><br />';	
			$output .= '<table>';
		...
		if($this->viewDay == $this->currentDay)
		{
			...
			$output .= '<br><strong>Add Transaction</strong>';
	...
	function LoadTraderData( $atts)
	{
		...
		$output .= '<br><strong>' . $country . ' ' . $category . ' Day ' . $this->viewDay . ' </strong><br /><br />';
		if ($this->viewDay < $this->currentDay)
			$output .= '(day has ended)<br />';
		...
		{
			$theData = $wpdb->get_row("SELECT " . $selection_fields . " FROM " . $tableName . " WHERE day = " . $this->viewDay );
...
//					$skip = true;
				...
				if ($skip == false)
				{
					...
					if( isset( $theData->$field))
					{
						...
						$output .= '<tr><td><br><strong>' . $prop->description .' ' . $num_items . ' ' .  $prop->plural . '</strong>,';
	...
	function LoadJobData( $atts)
	{
		...
		{
			$armyData = $wpdb->get_row("SELECT " . $selection_fields . " FROM " . $tableName . " WHERE day = " . $this->viewDay );
			// explode doesn't work?  I don't know why
			$first_word_ends = strpos($_POST['change_numbers'], " ");
		...
		$armyData = $wpdb->get_row("SELECT " . $selection_fields . " FROM " . $tableName . " WHERE day = " . $this->currentDay );
		...
		{	
			...
			{	
				...
				$output .= '<tr><td><br><strong>' . $prop->description .' ' . $num_items . ' ' .  $prop->name . 's</strong>,';
				$output .= $prop->value_pretty_print . 'Total: $' . $total_item_price . '</td>';
	...
	function LoadPopulationData( $atts )
	{
		...
		$output .= '<br><strong>' . $country . ' Population Day ' . $this->viewDay . ' </strong><br /><br />';
		...
		$data = $wpdb->get_row("SELECT " . $selection_fields . " FROM " . $tableName . " WHERE day = " . $this->viewDay  );
		$readonly = false;
		...
		{	
			...
			$output .= '</table>';
			if( $this->currentDay == $this->viewDay)
				$output .= '<br><input type="submit" name="update" value="Update Number of Cities"><br>';
		}	
		$output .= $this->LoadFooter();		
		return $output;
	...
	function LoadCentralBank()
	{
		...
		$bankData = $wpdb->get_row("SELECT " . $selection_fields . " FROM central_bank WHERE day = " . $this->currentDay . " ORDER BY id" );
		if($bankData)
		{	
			$output .= '<form method="post" action="">';
			$output .= '<table>';
			...
			if( $this->viewDay == $this->currentDay)
				$output .= '<br><input type="submit" name="update" value="Update Accounts">';
			$output .= '</form>';
		}
	...
	function LoadBankData( $atts )
	{
		...
		$output .= '<br><strong>' . $country . ' ' . 'Bank Account Day ' . $this->currentDay . ' </strong><br /><br />';
		$country = strtolower($country);
		...
		$bankData = $wpdb->get_row("SELECT " . $selection_fields . " FROM " . $tableName . " WHERE day = " . $this->currentDay . " ORDER BY id" );
	...
	function LoadTaxData( $atts )
	{
		...
		if (isset($_POST['update']))
		{
			...
			$productionData = $wpdb->get_row("SELECT total_production FROM $industryTable WHERE day=$this->currentDay");
			$popTable = $country . '_population_log';
			$num_cities = $wpdb->get_var("SELECT cities FROM $popTable WHERE day=$this->currentDay");
			
			
			$selection_fields .= 'total_taxes';
			$taxData = $wpdb->get_row("SELECT " . $selection_fields . " FROM " . $tableName . " WHERE day = " . $this->currentDay  );
		...
		$output .= '<br><strong>' . $country . ' Taxes Day ' . $this->viewDay . ' </strong><br /><br />';
		$country = strtolower( $country );
		$tableName = $country . '_taxes';
		...
		{	
			...
			$output .= '</table>';
			if( $this->viewDay == $this->currentDay)
				$output .= '<br><input type="submit" name="update" value="Update Percentages">';
			$output .= '</form>';
		}
		$output .= $this->LoadFooter();
		return $output;
	...
	function LoadGovtProgramData( $atts )
	{
		...
		if (isset($_POST['update']))
		{
			...
			$taxData = $wpdb->get_row("SELECT " . $tax_selection . " FROM " . $taxTable . " WHERE day = " . $this->currentDay  );			
			$income_tax_recd = ($taxData->income_tax_rate/100) * $productionData->total_production;
			$property_tax_recd = ($taxData->property_tax_rate/100) * $num_cities * 10000 * 50000;	
			$sales_tax_recd = ($taxData->sales_tax_rate/100) * $productionData->total_production;
			$total_tax_recd = $income_tax_recd + $property_tax_recd + $sales_tax_recd;
		
			$progData = $wpdb->get_row("SELECT " . $selection_fields . " FROM " . $tableName . " WHERE day = " . $this->currentDay  );
		...
		$output .= '<br><strong><br>' . $country . ' Government Programs Budget Day ' . $this->viewDay . ' </strong><br /><br />';
		$country = strtolower( $country );
		$tableName = $country . '_govprograms_log';
	...
	function LoadConsumptionData( $atts)
	{
		...
		$output = $this->LoadCountryHeader( $country );
		$output .= '<br><strong>' . $country . ' Production and Consumption Day ' . $this->viewDay . ' </strong><br /><br />';
		$country = strtolower($country);
		...
		$consumptionData = $wpdb->get_row("SELECT $selection_fields FROM $tableName,  $industryTable WHERE $tableName.day = $this->viewDay AND $industryTable.day = $this->viewDay" );
	...
	function carryValuesForward( $country, $category, $oldDay, $newDay)
	{
		...
		$theData = $wpdb->get_row("SELECT " . $selection_fields . " FROM " . $tableName . " WHERE day = " . $oldDay );
		$oldData = $theData;
		if($oldDay != 0)
		{
			$superOldDay = $oldDay -1;
			$oldData = $wpdb->get_row("SELECT " . $selection_fields . " FROM " . $tableName . " WHERE day = " . $oldDay );
		}
		$property_name = $category . '_properties';
		...
		if( $add_consumption  )
			$category_properties = $wpdb->get_results("SELECT description, name, plural, field_name, unit, value_per_unit, value_pretty_print, initial_energy_cost, ongoing_energy_cost, initial_construction_cost FROM $property_name");
	...
	function initializeBank( $country, $oldDay, $newDay)
	{
		...
		if( $this->isCountry( $country ))
		{
			$oldBalance = $wpdb->get_var("SELECT account_balance FROM $tableName WHERE day=$oldDay");
	...
	function updateTaxesAndGovtProgs( $country, $oldDay, $newDay)
	{
		...
		$selection_fields .= 'total_taxes';
		$taxData = $wpdb->get_row("SELECT " . $selection_fields . " FROM " . $taxTable . " WHERE day = " . $oldDay  );
		...
		$popData = $wpdb->get_row("SELECT " . $selection_fields . " FROM " . $popTable . " WHERE day = " . $oldDay  );
		
		
		$industryTable = $country . '_industry_log';
		$productionData = $wpdb->get_row("SELECT total_production FROM $industryTable WHERE day=$oldDay");
		...
		$progData = $wpdb->get_row("SELECT " . $selection_fields . " FROM " . $progTable . " WHERE day = " . $oldDay  );
		if($progData)
		{
			$money_paid = 0;
			$total_money = 0;
...
*/
	...
	function calculateProduction( $country, $day)
	{
		...
		$industry_data = $wpdb->get_row("SELECT " . $industrySelection . " FROM " . $industryTable .  " WHERE day= " . $day );
	...
	/*	if($day == 0)
		{
			$popTable = $country . '_population_log';
			$populationData = $wpdb->get_row("SELECT population, unemployment_rate FROM $popTable WHERE day=0");
	...
	function updatePopulationAndConsumption($country, $oldDay, $newDay)
	{
		...
		$selection_fields = '';
		$homeTable = $country . '_home';
		$liveable_wage = $wpdb->get_var("SELECT liveable_wage FROM $homeTable WHERE day=0");
		$productionData = $wpdb->get_row("SELECT total_production, alt_energy_produced FROM $industryTable WHERE day=$oldDay");
		...
		$data = $wpdb->get_row("SELECT " . $selection_fields . " FROM " . $popTable . " WHERE day = " . $oldDay  );
		...
		{	
			...
			{
				$intelibus_unemployment = $wpdb->get_var("SELECT unemployment_rate FROM intelibus_population_log WHERE day= ".$oldDay );
	...
	function closeDay()
	{
		...
		// army_properties
		$output = '<br><strong>close Day ' . $this->currentDay .'</strong><br />';
		foreach($this->entities as $ent)
		{
			$skip = false;
			...
			if ( $this->isCountry( $country) )
			{
				...
				$energy_used = $wpdb->get_var( "SELECT total_energy_consumed FROM $consumptionTable WHERE day = $newDay");
				$alt_produced = $wpdb->get_var( "SELECT alt_energy_produced FROM $industryTable WHERE day = $newDay");
				$alt_used = 0;
				$oil_used = 0;
	...
	{	
		...
		if( isset($_POST['EnterNotes']) )
		{
			for($ii = 0; $ii < $_POST['notesCount']; $ii++)
			{
				...
				$wpdb->update('scenarios', array('Notes' => $_POST[$noteName]), 
													array('id'=> $sceneId), 
													array('%s'), array('%d'));
			}
			$this->scenarios = $wpdb->get_results("SELECT id, Title, scene, Notes FROM scenarios WHERE day=" . $this->viewDay );
		...
		if( isset($_POST['endDay']) )
		{
			...
			$newDay = $this->currentDay + 1;
			$wpdb->query(" UPDATE game_state SET current_day = $newDay, viewing_day = $newDay WHERE id = 1");
			
			$this->InitDB();
		}
		if( isset($_POST['viewPast']) )
		{
			$wpdb->query(" UPDATE game_state SET viewing_day = " . $_POST['viewDay'] . " WHERE id = 1");
			$this->InitDB();
		...
		$output .= '<br><strong>Welcome to Almost Utopia<br />';
...
//		$output .= '<tr><td></td><td><a href=../commodity-values/>Commodity Values</a></td></tr></table>';
		...
		$output .= '<br /><br><strong>Events</strong><br />';
		$output .= '<form method="post" action="">';
		if( $this->scenarios)
		{
			$sceneCount = 0;
			...
			{
				$output .= '<br /><b>' . $event->Title;
				$output .= '</b><br />' . $event->scene;
				$varName = 'Scene' . $sceneCount;
				$output .= '<input type="hidden" name="' . $varName . '" value="'. $event->id . '" />';
				$textAreaName = 'Note' . $sceneCount;
				$output .= '<br><textarea name="' . $textAreaName . '" cols="100" rows="5"';
				if( $this->currentDay != $this->viewDay)
					$output .= ' readonly="readonly" ';
				$output .= '>';
				$output .= $event->Notes;
				$output .= '</textarea><br>';
				$output .= '<input type="submit" value="Enter Notes" name="EnterNotes" /><br>';
				$sceneCount++;
			}
			$output .= '<br><input type="hidden" name="notesCount" value = "' . $sceneCount . '" />';
		}
		...
		$output .= '</td><td>';
		$output .= 'View Day <select name="viewDay">';
...
//		$output .= '</form><br/>';
	...
	function LoadCountryPage( $atts )
	{
		...
		$output .= '<br><strong>Welcome to ' . $country . '.  Day ' . $this->viewDay;
		$readonly = false;
		if ($this->viewDay == $this->currentDay)
		{
			$output .= ' (today).<br />';
		...
		$dayWeather = '';
		$dayWeather = $wpdb->get_var("SELECT description FROM available_weather, $tableName WHERE available_weather.id = $tableName.weather_id AND $tableName.day  = $this->viewDay");		
		
		$output .= '<form method="post" action="">';
		$output .= '<br>Weather Report <br><br />';
...
//		$liveable_wage = $wpdb->get_var("SELECT liveable_wage FROM $tableName WHERE day=0");
